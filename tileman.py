...
threadpool.setMaxThreadCount(1500)
	...
	if tiles:
		for tile in tiles:
			try:
				...
				pass
	del tiles[:]
	print('scanning for tiles in the networks')
	host_adresses = socket.gethostbyname_ex(socket.getfqdn())[2]
	print(host_adresses)
	for adress in host_adresses:
		adress_split = adress.split('.')
		if len(adress_split) == 4:
			subnet_adress = adress_split[0]+'.'+adress_split[1]+'.'+adress_split[2]+'.'
